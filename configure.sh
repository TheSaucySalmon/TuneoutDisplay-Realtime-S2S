...
_def="${OPENAI_API_KEY:-}"
if [ -n "$_def" ]; then
    read -rsp "  OpenAI API key      (press ENTER to keep saved): " _in
    echo ""
...
_def="${OPENAI_REALTIME_MODEL:-gpt-realtime}"
...
_def="${OWW_MODEL:-hey_jarvis}"
read -rp "  Wake word model         [${_def}]: " _in
OWW_MODEL="${_in:-${_def}}"

_def="${OWW_THRESHOLD:-0.5}"
...
# The active assistant runtime is intentionally left to a follow-up step, so
# mic.sh is not part of the normal boot path. It is kept here as a handy diagnostic
# tool: running mic.sh | aplay confirms the hardware capture path is working
# independently of the future voice runtime.
MIC_SCRIPT="$CURRENT_HOME/mic.sh"

cat > "$MIC_SCRIPT" << 'MICEOF'
#!/bin/bash
# Records stereo 48kHz (WM8960 native rate) and converts to 16kHz mono.
# Kept for diagnostics — not used by the main setup flow.
...
# dmix creates a software mixer so future assistant playback and sendspin
...
# seeed_tts    softvol over seeed_shared — assistant/TTS playback (independent vol)
# seeed_media  softvol over seeed_shared — Music Assistant playback (independent vol)
#
# Future assistant playback should target seeed_tts; sendspin uses seeed_media.
...
# PipeWire (via pipewire-pulse) provides a local audio server that a future
# assistant runtime can use for microphone input. Playback stays on the raw ALSA softvol
...
# a future assistant runtime can read the microphone via the PipeWire-Pulse source.
...
# The seeed capture (mic) node remains enabled for local microphone access.
monitor.alsa.rules = [
  {
    matches = [
...
# TTS Volume  → controls assistant/voice playback level (seeed_tts softvol device)
...
## 1. MQTT Entities (Auto-Created — No YAML Needed)
...
> The volume, brightness, and mic sensitivity controls work immediately via MQTT.
> The status chip and mute toggle still depend on compatible HA entities being
> provided by your assistant runtime.
...
## 5. Assistant Runtime Config

The base setup also saves a starter assistant config file at:
...
- Device name and ID
- local assistant state and memory paths
