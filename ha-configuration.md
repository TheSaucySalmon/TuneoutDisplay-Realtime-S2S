...
This reference preserves the existing Lovelace pattern, but the `assist_satellite.*`
and mute/status entities must be supplied by a compatible assistant runtime if you
remove Linux Voice Assistant from the device.
...
> The `number.*` entities come from the MQTT bridge. The `assist_satellite.*` status entity
> must come from a compatible assistant integration if you want to keep the current card behavior.
...
Verify actual IDs in HA under **Settings → Devices & Services**.
